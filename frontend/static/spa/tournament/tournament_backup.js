function ensureHumanFirst(player1, player2, gameManager) {
  ...
  function initializeTournamentPage() {
    ...
    class TournamentGameManager {
      ...
      //   isBot(player) {
      //     if (!player) return false; // Assurez-vous que le joueur existe
  
      //     // Trouver l'index du joueur dans la liste des joueurs
      //     const playerIndex = this.tournamentState.players.findIndex(p => p.name === player.name);
  
      //     if (playerIndex === -1) {
      //         console.error("Player not found in tournamentState.players:", player);
      //         return false; // Si le joueur n'est pas trouvé
      //     }
  
      //     // Trouver la case à cocher associée dans le DOM
      //     const botCheckbox = document.querySelectorAll('.bot-checkbox')[playerIndex];
  
      //     if (!botCheckbox) {
      //         console.error("No bot-checkbox found for playerIndex:", playerIndex);
      //         return false;
      //     }
  
      //     // Retourner l'état de la case à cocher
      //     console.log(`Player ${player.name} isBot: ${botCheckbox.checked}`);
      //     return botCheckbox.checked;
      // }
      ...
      //     // Activer l'IA si nécessaire
      //     if (player1IsBot || player2IsBot) {
      //       const options = null; // Ajouter des options spécifiques si nécessaires
      //       this.gameContainer.contentWindow.postMessage(
      //           {
      //               type: "setOptions",
      //               data: {
      //                   isAI: true,
      //                   power: true, // Ex. : puissance de l'IA
      //                   botPlayer: player1IsBot ? "player1" : "player2",
      //               },
      //           },
      //           "*"
      //       );
      ...
      //     // Déterminer si chaque joueur est un bot
      //     const player1IsBot = this.isBot(currentMatch.player1);
      //     const player2IsBot = this.isBot(currentMatch.player2);
  
      //     console.log("Starting match with player statuses:");
      //     console.log("Player 1:", currentMatch.player1.name, "isBot:", player1IsBot);
      //     console.log("Player 2:", currentMatch.player2.name, "isBot:", player2IsBot);
  
      //     // Charger le conteneur de jeu
      //     this.gameContainer.style.display = "block";
      //     this.gameContainer.src = "/static/spa/game3D/three.html";
      ...
      //         // Envoyer les informations des joueurs au jeu
      //         this.gameContainer.contentWindow.postMessage(
      //             {
      //                 type: "startGame",
      //                 data: {
      ...
      //         // Définir les options si l'un des joueurs est un bot
      //         if (player1IsBot || player2IsBot) {
      //             const botPlayer = player1IsBot ? "player1" : "player2";
      //             const options = null; // Ajouter des options spécifiques si nécessaires
      //             this.gameContainer.contentWindow.postMessage(
      //                 {
      //                     type: "setOptions",
      ...
      startGame() {
        ...
        // // Réorganiser les joueurs pour que l'humain soit toujours player1
        // if (player1IsBot && !player2IsBot) {
        //   console.log(`Swapping players to make human player1.`);
        //   // Swap player1 and player2
        //   const tempPlayer = currentMatch.player1;
        //   currentMatch.player1 = currentMatch.player2;
        //   currentMatch.player2 = tempPlayer;
        // }
        ...
        this.gameContainer.onload = () => {
          ...
          // Par défaut, on ne change pas les options si ce ne sont pas des bots
    ...
    function generateMatches() {
      ...
      // const ensureHumanFirst = (player1, player2, gameManager) => {
      //   const player1IsBot = gameManager.isBot(player1);
      //   const player2IsBot = gameManager.isBot(player2);
      //   // Si player1 est un bot et player2 est un humain, échangez-les
      //   if (player1IsBot && !player2IsBot) {
      ...
      if (playerCount === 4) {
        if (tournamentState.currentMatch < 2) {
          const [newPlayer1, newPlayer2] = ensureHumanFirst(
            currentMatch.winner,
            tournamentState.matches[2]?.player2,
            gameManager
          );
          tournamentState.matches[2].player1 = newPlayer1;
          tournamentState.matches[2].player2 = newPlayer2;
        }
      } else if (playerCount === 8) {
        if (tournamentState.currentMatch < 4) {
          ...
          tournamentState.matches[nextRoundMatchIndex].player1 = newPlayer1;
          tournamentState.matches[nextRoundMatchIndex].player2 = newPlayer2;
        }
      }
  
      // Affichage des résultats et progression
      if (isLastMatch) {
    ...
    //     currentMatch.winner = currentMatch.player2;
    //   } else {
    //     currentMatch.winner = null; // En cas d'égalité, aucun gagnant
    //   }
    ...
    //     // Fonction pour repositionner humain en player1 si nécessaire
    //   const ensureHumanFirst = (player1, player2) => {
    //     const player1IsBot = isBot(player1);
    //     const player2IsBot = isBot(player2);
    //     if (player1IsBot && !player2IsBot) {
    //       return [player2, player1];
    //     }
    //     return [player1, player2];
    ...
    //         tournamentState.matches[2].player1 = ensureHumanFirst(currentMatch.winner, tournamentState.matches[2].player2, gameManager);
    //       } else {
    //         tournamentState.matches[2].player2 = ensureHumanFirst(tournamentState.matches[2].player1, currentMatch.winner, gameManager);
    //       }
    //     } else if (tournamentState.currentMatch === 2) {
    //       // Finale
    //       const finalMatch = tournamentState.matches[2];
    //       finalMatch.winner = currentMatch.winner;
  
    //       // Créer le match virtuel pour l'affichage final
    //       tournamentState.matches[3] = {
    //         matchId: 3,
    //         round: 3,
    //         player1: currentMatch.winner,
    //         player2: null,
    ...
    //         tournamentState.matches[nextRoundMatchIndex].player1 = ensureHumanFirst(currentMatch.winner, tournamentState.matches[nextRoundMatchIndex].player2, gameManager);
    //       } else {
    //         tournamentState.matches[nextRoundMatchIndex].player2 = ensureHumanFirst(tournamentState.matches[nextRoundMatchIndex].player1, currentMatch.winner, gameManager);
    //       }
    //     } else if (tournamentState.currentMatch < 6) {
    //       // Demi-finales (matchs 4 et 5)
    //       if (tournamentState.currentMatch === 4) {
    //         tournamentState.matches[6].player1 = ensureHumanFirst(currentMatch.winner, tournamentState.matches[6].player2, gameManager);
    //       } else {
    //         tournamentState.matches[6].player2 = ensureHumanFirst(tournamentState.matches[6].player1, currentMatch.winner, gameManager);
    //       }
    //     } else if (tournamentState.currentMatch === 6) {
    //       // Finale
