...
function animate() {
  ...
  if (!scoreSystem.isGameOver()) {
    paddle1Controller.move(boundaries, gameStarted, keyboard, height1);
    if (!aiIsActive) {
      paddle2Controller.move(boundaries, gameStarted, keyboard, height2);
    }
  }
  ...
  if (isBallMoving && !scoreSystem.isGameOver()) {
    ...
    if (ballVelocity.x < 0) {
      ...
      if (
        ...
        ball.position.x >= paddleLeftX - 30 &&
        Math.abs(ball.position.z - paddle1.position.z) < height1 / 1.8
      ) {
    ...
    if (ballVelocity.x > 0) {
      ...
      if (
        ...
        ball.position.x <= paddleRightX + 30 &&
        Math.abs(ball.position.z - paddle2.position.z) < height2 / 1.8
      ) {
        currentBallSpeed = Math.min(
          currentBallSpeed + SPEED_INCREMENT,
          MAX_BALL_SPEED
