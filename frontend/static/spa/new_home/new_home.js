...
function initializeHome() {
  ...
  class ChatHandler {
	...
	class ContextMenu {
      ...
      document.addEventListener("click", (e) => {
        if (!downLeftFrame.contains(e.target)) {contextMenuElement.style.display = "none";
			}
		});
    }
      ...
      `;
			...
			homePageMain.appendChild(confirmation);setTimeout(() => confirmation.remove(), 2000);
		}
	}
	...
	class GameOptionsManager {
		static initialize() {
          ...
          option.classList.add("option-selected");
			...
			DOM.game.modeOptions.forEach((option) => {if (option.textContent.trim() === "AGAINST AI") {
          option.classList.add("option-selected");
        }
				option.addEventListener("click", () =>
					handleSelection(DOM.game.modeOptions, option)
