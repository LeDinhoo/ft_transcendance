...
const wsManager = {
		...
		this.chatSocket.onmessage = (e) => {
			...
			switch (data.type) {
				case 'message_history':
					console.log("Réception de l'historique:", data.messages.length, "messages");
					const chatMessages = document.getElementById('chatMessages');
					if (chatMessages) {
						chatMessages.innerHTML = '';
						this.messageHistory = [];

						// Les messages de l'historique sont déjà filtrés par le serveur
				...
				case 'chat_message':
					// Stocker dans l'historique local seulement les messages publics
					this.messageHistory.push(data);
				...
				case 'private_message':
					// Ne pas stocker les messages privés dans l'historique
	...
	handleMessage(data) {
		...
		// CHANGEMENT 1 : Vérification sécurisée de ChatHandler
		const isBlocked = window.ChatHandler?.blockedUsers?.has(data.username) || false;
		...
		// CHANGEMENT 2 : Utilisation de isBlocked au lieu de ChatHandler direct
		if (isBlocked) {
