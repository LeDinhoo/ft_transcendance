...
class World extends EventTarget {
	...
	internalStep(dt) {
		...
		if (doProfiling) {
			profil.broadphase = performance.now() - profilingStart;
		...
		if (doProfiling) {
			profil.narrowphase = performance.now() - profilingStart;
		...
		if (doProfiling) {
			profil.makeContactConstraints = performance.now() - profilingStart;
			profilingStart = performance.now();
		} // Wake up bodies
		...
		if (doProfiling) {
			profil.solve = performance.now() - profilingStart;
		...
		if (doProfiling) {
			profil.integrate = performance.now() - profilingStart;
		} // Update step number
