...
function initializeGameElements() {
  ...
  console.log(
    "Dimensions gameSection :",
    gameElements.gameSection.offsetWidth,
    gameElements.gameSection.offsetHeight
  );
  console.log(
    "Dimensions topPaddle :",
    gameElements.topPaddle.offsetWidth,
    gameElements.topPaddle.offsetHeight
  );
  console.log(
    "Dimensions ball :",
    gameElements.ball.offsetWidth,
    gameElements.ball.offsetHeight
  );
...
function setupEventListeners() {
  gameElements.gameSection.addEventListener(
    "mouseenter",
    () => (gameState.isMouseInGame = true)
  );
  gameElements.gameSection.addEventListener(
    "mouseleave",
    () => (gameState.isMouseInGame = false)
  );
...
function updateBallAppearance() {
  let currentSpeed = Math.sqrt(
    gameState.ballSpeedX ** 2 + gameState.ballSpeedY ** 2
  );
  let normalizedSpeed = Math.pow(
    (currentSpeed - MIN_SPEED) / (MAX_SPEED - MIN_SPEED),
    1 / gameState.colorSensitivity
  );
...
function moveTopPaddle() {
  ...
  if (gameState.isTopPaddleControlledByKeyboard) {
    ...
    if (gameState.rightKeyPressed) {
      gameState.topPaddleX = Math.min(
        gameState.gameWidth - PADDLE_WIDTH,
        gameState.topPaddleX + PADDLE_SPEED
      );
    }
  } else {
    gameState.topPaddleX = moveAIPaddle(
      gameElements.topPaddle,
      gameState.topPaddleX,
      true
    );
  }
...
function moveBottomPaddle() {
  ...
  if (gameState.isBottomPaddleControlledByKeyboard) {
    if (gameState.aKeyPressed) {
      gameState.bottomPaddleX = Math.max(
        0,
        gameState.bottomPaddleX - PADDLE_SPEED
      );
    }
    if (gameState.dKeyPressed) {
      gameState.bottomPaddleX = Math.min(
        gameState.gameWidth - PADDLE_WIDTH,
        gameState.bottomPaddleX + PADDLE_SPEED
      );
    }
  } else {
    gameState.bottomPaddleX = moveAIPaddle(
      gameElements.bottomPaddle,
      gameState.bottomPaddleX,
      false
    );
  }
...
function moveBall() {
  ...
  if (
    gameState.ballX <= 0 ||
    gameState.ballX >= gameState.gameWidth - BALL_SIZE
  ) {
...
function checkPaddleCollisions() {
  ...
  if (
    gameState.ballX + BALL_SIZE > topPaddlePosition.left &&
  ...
  if (
    gameState.ballX + BALL_SIZE > bottomPaddlePosition.left &&
...
function moveAIPaddle(paddle, paddleX, isTopPaddle) {
  ...
  if (
    (isTopPaddle && gameState.ballSpeedY < 0) ||
    (!isTopPaddle && gameState.ballSpeedY > 0)
  ) {
    let timeToIntercept = Math.abs(
      (paddle.offsetTop - gameState.ballY) / gameState.ballSpeedY
    );
...
function updateColorSensitivity(newSensitivity) {
  ...
  console.log(
    `Sensibilité de couleur mise à jour : ${gameState.colorSensitivity}`
  );
}
