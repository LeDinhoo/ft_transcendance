...
def register_view(request):
    try:
        ...
        if register_form.is_valid():
            ...
            logger.warning(
                f"Erreurs dans le formulaire : {register_form.errors}")
            return JsonResponse(
                {
                    'success': False,
...
def update_profile_view(request):
    ...
    if 'email' in data:
        ...
        try:
            ...
            return JsonResponse({'error': 'L\'adresse email est invalide.'},
                                status=400)
    ...
    if 'old_password' in data and 'new_password' in data:
        ...
        if not check_password(old_password, user.password):
            return JsonResponse(
                {'error': 'L\'ancien mot de passe est incorrect.'}, status=400)
    ...
    if 'avatar' in request.FILES:
        ...
        if ext not in valid_image_extensions:
            return JsonResponse(
                {
                    'error':
                    'Seuls les fichiers PNG, JPG ou JPEG sont acceptés.'
                },
                status=400)
        user.avatar = avatar
        ...
        if selected_avatar.startswith(expected_prefix):
            ...
            return JsonResponse(
                {
                    'error':
                    f'Chemin d\'avatar invalide. Le chemin doit commencer par {expected_prefix}'
                },
                status=400)
...
def logout_view(request):
    try:
        ...
        logger.error(
            f"Erreur lors du blacklistage du refresh token : {str(e)}")
...
def refresh_token_view(request):
    ...
    if not refresh_token:
        return JsonResponse({'error': 'Refresh token not found in cookies'},
                            status=403)
    ...
    try:
        ...
        return JsonResponse({'error': 'Invalid or expired refresh token'},
                            status=403)
...
def get_auth_url(request):
    ...
    try:
        ...
        response = JsonResponse({'success': True, 'auth_url': auth_url})
...
def callback_42(request):
    try:
        ...
        try:
            ...
            avatar_url = user_data.get('image', {}).get('versions',
                                                        {}).get('large')
            logger.info(f"Found avatar URL: {avatar_url}")

            user = CustomUser.objects.filter(
                intra_42_id=user_data['id']).first()
            ...
            if avatar_url and user.is_42_user and (
                    not user.avatar
                    or user.avatar.name == 'assets/avatars/ladybug.png'):
                try:
    ...
    def post(self, request):
        ...
        if action == 'enable':
            ...
            if send_2fa_email(user, code):
                ...
                return Response({'error': "Erreur lors de l'envoi de l'email"},
                                status=status.HTTP_500_INTERNAL_SERVER_ERROR)
            ...
            return Response({'error': "2FA n'est pas activé"},
    ...
    def post(self, request):
        ...
        if user.two_factor_code_timestamp and \
           timezone.now() > user.two_factor_code_timestamp + timedelta(minutes=10):
            return Response({'error': 'Code expiré'},
                            status=status.HTTP_400_BAD_REQUEST)
    ...
    try:
        ...
        if not code:
            ...
            return JsonResponse(
                {
                    'success': False,
                    'message': 'Le code 2FA est requis.'
                },
                status=400)
        ...
        if request.user.is_authenticated:
            ...
            if not user_id:
                logger.warning(
                    "Aucun identifiant utilisateur fourni pour la connexion.")
                return JsonResponse(
                    {
                        'success':
                        False,
                        'message':
                        "L'identifiant utilisateur est requis pour cette opération."
                    },
                    status=400)
        ...
        if not user.two_factor_code or not user.two_factor_code_timestamp:
            logger.warning(
                "Aucun code 2FA actif trouvé pour l'utilisateur : %s",
                user.email)
            return JsonResponse(
                {
                    'success': False,
        ...
        if timezone.now() > user.two_factor_code_timestamp + timedelta(
                minutes=10):
            logger.warning("Code 2FA expiré pour l'utilisateur : %s",
                           user.email)
            return JsonResponse({
                'success': False,
                'message': 'Code expiré.'
        ...
        if code == user.two_factor_code:
            ...
            if not request.user.is_authenticated:
                ...
                logger.info("Génération des tokens pour l'utilisateur : %s",
                            user.email)
                refresh = RefreshToken.for_user(user)
                access_token = str(refresh.access_token)
                refresh_token = str(refresh)
                ...
                logger.info("Activation du 2FA pour l'utilisateur : %s",
                            user.email)
                user.is_2fa_enabled = True
                response = JsonResponse({
                    'success': True,
            ...
            logger.warning("Code 2FA invalide pour l'utilisateur : %s",
                           user.email)
            return JsonResponse({
                'success': False,
        ...
        logger.error(
            f"Erreur inattendue lors de la vérification 2FA : {str(e)}")
        return JsonResponse(
            {
                'success': False,
...
def record_game(request):
    ...
    if max_ball_speed is None:
        return JsonResponse(
            {'error': 'Données incorrectes : max_ball_speed manquant'},
            status=400)

    if not all([
            score_user is not None, score_opponent is not None, result
            is not None
    ]):
    ...
    if opponent_id:
        try:
            ...
            return JsonResponse({'error': 'Adversaire introuvable'},
                                status=404)

    user_longest_rally = GameHistory.objects.filter(
        user=request.user).aggregate(
            Max('longest_rally'))['longest_rally__max'] or 0

    if longest_rally > user_longest_rally:
        print(
            f"Mise à jour du longest rally : {longest_rally} (ancien : {user_longest_rally})"
        )

    user_max_ball_speed = GameHistory.objects.filter(
        user=request.user).aggregate(
            Max('max_ball_speed'))['max_ball_speed__max'] or 0

    if max_ball_speed > user_max_ball_speed:
        print(
            f"Mise à jour du max ball speed : {max_ball_speed} (ancien : {user_max_ball_speed})"
        )
    ...
    game = GameHistory.objects.create(
        ...
        if longest_rally > user_longest_rally else user_longest_rally,
        opponent_user=opponent_user,
        opponent_name=opponent_name if not opponent_user else None,
        max_ball_speed=max_ball_speed
        if max_ball_speed > user_max_ball_speed else user_max_ball_speed,
    )
    ...
    for game in games:
        ...
        if game.opponent_user:
            if game.opponent_user.avatar:
                if str(game.opponent_user.avatar).startswith(
                        'assets/avatars/'):
...
def get_user_statistics(request):
    ...
    power_catch_avg = GameHistory.objects.filter(user=user).aggregate(
        Avg('power_catch'))['power_catch__avg']
    max_ball_speed = GameHistory.objects.filter(user=user).aggregate(
        Max('max_ball_speed'))['max_ball_speed__max'] or 0
    longest_rally = GameHistory.objects.filter(user=user).aggregate(
        Max('longest_rally'))['longest_rally__max'] or 0
...
def send_friend_request(request):
    ...
    try:
        ...
        existing_request = FriendShip.objects.filter(from_user=request.user,
                                                     to_user=receiver).first()
        ...
        if existing_request:
            if existing_request.status == 'pending':
                ...
                return JsonResponse({'message': 'You are already friends'},
                                    status=400)

        FriendShip.objects.create(from_user=request.user,
                                  to_user=receiver,
                                  status='pending')
...
def get_friends(request):
    try:
        ...
        for friendship in friendships:
            ...
            friend_avatar = (f"/static/{friend.avatar}" if str(
                friend.avatar).startswith('assets/avatars/') else
                             friend.avatar.url if friend.avatar else
...
def get_user_profile_stats(request, user_id):
    try:
        ...
        logger.debug(
            f"Stats: max_ball_speed={max_ball_speed}, longest_rally={longest_rally}"
        )

        # Déterminer le rang
        if win_ratio >= 80 and total_games >= 5:
            rank = "Platinium"
        elif (win_ratio >= 66 and win_ratio < 80) or (win_ratio >= 66
                                                      and total_games < 5):
            rank = "Gold"
...
def get_blocked_users(request):
    ...
    blocked_list = [{
        ...
        user.username,
        'avatar':
        f"/static/{user.avatar}"
        if str(user.avatar).startswith('assets/avatars/') else user.avatar.url
        if user.avatar else '/static/assets/avatars/ladybug.png',
